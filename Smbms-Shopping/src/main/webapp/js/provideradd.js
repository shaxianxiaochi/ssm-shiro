...
$(function () {
    ...
    proCode.on("blur", function () {
        if (proCode.val() != null && proCode.val() != "") {
            var url = path+"/provider/checkProCode.action";
            var params = {
                "proCode": $("#proCode").val()
            };
            $.post(url, params, function (data) {
                if (data.status == "200") {
                    ...
                    validateTip(proCode.next(), {"color": "red"}, imgNo + " 编码已经被使用！", false);
                }
            },"JSON");
        } else {
 ...
 */
function submit() {
    var url = path+"/provider/addProvider.action";
    var params = {
        "proCode": $("#proCode").val(),
        "proName": $("#proName").val(),
        "proDesc": $("#proDesc").val(),
